...
- **평점 잠금** — 로그아웃 상태에서 전문의 상세 응답에 평점 **숫자**가 없는가 (개발자 도구 네트워크 탭 확인).
  계약이 요구하는 형태는 **키 삭제가 아니라 `"rating": null`** 이다 — `openapi.yaml` 의 `Doctor.rating` 은
  `type: [number, 'null']` 이고 `required` 목록에 없다. 키가 사라졌는지로 판정하면 통과해야 할 구현이 실패한다
