...
| **수정** 화면에서 대표 이미지를 바꿔도 상세 캐러셀이 안 바뀐다 | 폼이 `images` 배열을 편집하지 않는다. `PATCH /hospitals/{id}` 에 `images` 가 있으니 **폼이 보내면 고쳐진다** — 화면 작업. **등록** 화면은 2026-08-14 에 `images: [thumbnail]` 을 보내도록 고쳤다(캐러셀이 비지는 않는다) — 남은 것은 수정 화면뿐이다 |
