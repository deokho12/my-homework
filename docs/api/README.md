...
| 조건 `원데이` | **서버** | 같음 |
| 조건 `진료시간` (야간상담) | **서버** | 같음 |
| 조건 `전문의` | **서버 (필수)** | **병원↔전문의 조인이 필요하다.** "인증 완료된 전문의가 1명 이상 있는 병원" 을 클라이언트에서 판정하려면 전문의 전량(현재 14명, 실서비스에서는 수천 명)을 받아야 한다. 지금 코드가 실제로 그렇게 하고 있다 (`useDoctorStore` 전체를 읽어 `doctorsByHospitalId` 맵을 만든다) |
...
현재는 `frontend/src/utils/sponsorship.ts` 가 클라이언트에서 `new Date()` 로 오늘을 구해 판단한다.
서버로 옮기는 이유:
