...
describe('인가 (e2e)', () => {
  ...
  describe('1층 인증 — 토큰이 없거나 못 믿을 때', () => {
    it('비로그인 요청은 보호된 경로에서 401 UNAUTHENTICATED', async () => {
      const response = await request(app.getHttpServer()).get('/api/v1/test-guards/authenticated').expect(401);
    ...
    it('비로그인 요청은 담당 병원 자원에서도 401 이다 (403/404 로 정보가 새지 않는다)', async () => {
      ...
      await request(app.getHttpServer())
        .get(`/api/v1/test-guards/consult-requests/${SEED_FIXTURES.consultAtH1}`)
        .expect(401);
      await request(app.getHttpServer())
        .patch(`/api/v1/test-guards/hospitals/${SEED_FIXTURES.hospitalManagedByH1Admin}`)
        .expect(401);
    });
    ...
    it('Bearer 스킴이 아니거나 토큰이 비어 있으면 401', async () => {
      for (const header of ['', 'Bearer', 'Bearer    ', 'Basic abc', adminH1.accessToken]) {
        const response = await request(app.getHttpServer())
          .get('/api/v1/test-guards/authenticated')
  ...
  describe('2층 역할 — x-role 밖의 역할은 403', () => {
    it('user 역할은 운영자 전용 경로에서 403 FORBIDDEN', async () => {
      const response = await request(app.getHttpServer())
        .get('/api/v1/test-guards/doctor-verification-queue')
    ...
    it('hospital_admin 도 전문의 인증 검수 경로에서 403 — 남의 병원 전문의를 심사할 수 없다', async () => {
      ...
      await request(app.getHttpServer())
        .put(`/api/v1/test-guards/doctors/${SEED_FIXTURES.doctorAtH1}/verification`)
        .set('Authorization', bearer(adminH1.accessToken))
        .expect(403);
    });
    ...
    it('operator 는 전문의 검수가 된다 — 담당 병원과 무관하게 전 병원', async () => {
      ...
      for (const doctorId of [SEED_FIXTURES.doctorAtH1, SEED_FIXTURES.doctorAtH11]) {
        await request(app.getHttpServer())
          .put(`/api/v1/test-guards/doctors/${doctorId}/verification`)
          .set('Authorization', bearer(operator.accessToken))
          .expect(200);
      }
    });
  ...
  describe('3층 담당 범위 — 공개 자원은 403', () => {
    ...
    it('hospital_admin 이 담당이 아닌 병원을 수정하려 하면 403 HOSPITAL_NOT_MANAGED', async () => {
      ...
      const response = await request(app.getHttpServer())
        .patch(`/api/v1/test-guards/hospitals/${SEED_FIXTURES.hospitalNotManagedByH1Admin}`)
        .set('Authorization', bearer(adminH1.accessToken))
        .expect(403);
    ...
    it('담당이 아닌 병원의 전문의 수정도 403 HOSPITAL_NOT_MANAGED', async () => {
      const own = await request(app.getHttpServer())
        .patch(`/api/v1/test-guards/doctors/${SEED_FIXTURES.doctorAtH1}`)
        .set('Authorization', bearer(adminH1.accessToken))
        .expect(200);
      ...
      const other = await request(app.getHttpServer())
        .patch(`/api/v1/test-guards/doctors/${SEED_FIXTURES.doctorAtH11}`)
        .set('Authorization', bearer(adminH1.accessToken))
        .expect(403);
    ...
    it('없는 병원은 담당자에게도 운영자에게도 404 HOSPITAL_NOT_FOUND', async () => {
      for (const session of [adminH1, operator]) {
        const response = await request(app.getHttpServer())
          .patch('/api/v1/test-guards/hospitals/h-does-not-exist')
          .set('Authorization', bearer(session.accessToken))
          .expect(404);
    ...
    it('@Roles 를 빠뜨린 라우트도 담당 검사가 막는다 (fail closed)', async () => {
      const response = await request(app.getHttpServer())
        .patch(`/api/v1/test-guards/misconfigured/hospitals/${SEED_FIXTURES.hospitalManagedByH1Admin}`)
        .set('Authorization', bearer(user.accessToken))
        .expect(403);
  ...
  describe('3층 담당 범위 — 비공개 자원(상담)은 404', () => {
    ...
    it('hospital_admin 이 담당이 아닌 병원의 상담을 조회하면 404 다 (403 이 아니다)', async () => {
      const response = await request(app.getHttpServer())
        .get(`/api/v1/test-guards/consult-requests/${SEED_FIXTURES.consultAtH2}`)
        .set('Authorization', bearer(adminH1.accessToken))
        .expect(404);
    ...
    it('★ 없는 상담과 남의 병원 상담의 응답이 완전히 같다 — 순차 대입으로 건수를 셀 수 없다', async () => {
      ...
      const missing = await request(app.getHttpServer())
        .get(`/api/v1/test-guards/consult-requests/${SEED_FIXTURES.consultMissing}`)
        .set('Authorization', bearer(adminH1.accessToken));
    ...
    it('operator 는 전 병원 상담을 조회할 수 있다 (마스킹 대상이므로 managed=false)', async () => {
      for (const consultId of [SEED_FIXTURES.consultAtH1, SEED_FIXTURES.consultAtH2]) {
        const response = await request(app.getHttpServer())
          .get(`/api/v1/test-guards/consult-requests/${consultId}`)
          .set('Authorization', bearer(operator.accessToken))
          .expect(200);
  ...
  describe('의도된 비대칭 — operator 는 조회는 되고 상태 변경은 안 된다', () => {
    it('operator 의 상담 상태 변경은 403 FORBIDDEN', async () => {
      ...
      const response = await request(app.getHttpServer())
        .patch(`/api/v1/test-guards/consult-requests/${SEED_FIXTURES.consultAtH1}/status`)
        .set('Authorization', bearer(operator.accessToken))
    ...
    it('operator 의 상태 변경 거부는 상담 존재 여부를 조회하지 않는다 (없는 id 도 403)', async () => {
      ...
      const response = await request(app.getHttpServer())
        .patch(`/api/v1/test-guards/consult-requests/${SEED_FIXTURES.consultMissing}/status`)
        .set('Authorization', bearer(operator.accessToken))
        .send({ status: 'contacted' })
        .expect(403);
    ...
    it('hospital_admin 은 담당 병원 상담의 상태를 바꿀 수 있고, 남의 병원 상담은 404', async () => {
      ...
      const response = await request(app.getHttpServer())
        .patch(`/api/v1/test-guards/consult-requests/${SEED_FIXTURES.consultAtH2}/status`)
        .set('Authorization', bearer(adminH1.accessToken))
        .send({ status: 'contacted' })
        .expect(404);
