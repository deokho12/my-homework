 ...
 *
 * `AuthModule` 은 세 가지 이유로 import 한다:
 ...
 * 3. **`GET /admin/hospitals`(`AdminHospitalController`)의 인가 2층**(`AuthGuard`·`RolesGuard`)과
 *    `HospitalService.listForAdmin` 이 담당 병원 범위를 구하려고 주입받는 `UsersRepository`.
 *    이 경로는 `:hospitalId` 가 없어 `HospitalScopeGuard`/`@HospitalScope` 를 쓰지 않는다 —
 *    범위는 쿼리 결과(`scope: 'managed' | 'all'`)로 표현된다.
 *
...
@Module({
  ...
  controllers: [HospitalController, AdminHospitalController],
