...
/**
 * `GET /hospitals/:hospitalId/doctors` 가 `HospitalController` 에 살기 때문에
 * `DoctorModule` 을 import 한다 (경로 소유자가 병원이다). `DoctorModule` 은 이 모듈을
 * 다시 import 하지 않는다 — 순환 참조 방지 (`doctor.module.ts` 주석 참고).
 *
 ...
 */
@Module({
  imports: [DoctorModule, AuthModule],
