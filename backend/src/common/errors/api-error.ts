...
export const ERROR_CATALOG = {
  ...
  /**
   * `전문의 상담신청` 으로 넘어온 `doctorId` 가 그 병원 소속이 아니다.
   *
   * 404 가 아니라 422 인 이유: 전문의는 공개 리소스라 존재를 숨겨서 얻는 것이 없고,
   * 화면은 "그 전문의는 이 병원 소속이 아니다" 를 폼 오류로 붙여야 한다.
   */
  DOCTOR_NOT_AT_HOSPITAL: { status: 422, message: '그 전문의는 이 병원 소속이 아니에요' },
  /** 고른 시술을 그 병원이 취급하지 않는다. 병원마다 `procedureIds` 가 다르다. */
  PROCEDURE_NOT_OFFERED: { status: 422, message: '이 병원에서 다루지 않는 시술이에요' },

  // ------------------------------------------------------------------ 알림
  /**
   * **남의 알림과 없는 알림을 구분하지 않는다.** 관리자 알림 문구에는 고객 이름이
   * 들어 있어서(`김민준님이 상담을 신청했어요`), 존재 여부만으로도 알려 줄 것이 아니다.
   */
  NOTIFICATION_NOT_FOUND: { status: 404, message: '알림을 찾을 수 없어요' },
